...
web/                          前端：Vite + React + TypeScript + Tailwind CSS，构建产物 web/dist 由 server.py 托管（规范见 web/CLAUDE.md）
docs/                         规则与设计文档（中文），代码以文档为准
docs/images/                  README 用的截图
deploy/docker/                agent / 编排端镜像 Dockerfile（内置前端产物）+ compose 生成器
deploy/k8s/                   Namespace / RBAC / 编排端 Deployment / 前端 Deployment
tests/                        全部测试（unittest）
werewolf/
...
3. 动了前端（`web/`）：先按 `web/CLAUDE.md` 跑 `npm run check` 和 `npm run build`，再用 `uv run werewolf-server` 起服务，在桌面宽度和手机宽度下都手动过一遍
...
- 前端是 `web/` 下的 Vite + React + TypeScript + Tailwind CSS 工程，按 `web/CLAUDE.md` 和 `web/.claude/skills/` 的规范开发；视觉设计已定稿，不在前端改设计。
