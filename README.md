...
把任意一个座位的后端选成「真人玩家」，就是 11 个 agent + 你一个人。开局后页面上方多出你的座位面板：
底牌、只有你知道的消息（狼人频道、验人结果、用药情况）、轮到你时的操作表单。
agent 看到什么你就看到什么，一样的规则、一样的字数上限。

有真人在打的对局，**结束前不能开上帝视角**（服务端直接不给，不只是按钮变灰）；打完照常看完整复盘。
一桌最多 1 个真人座位，开局的那个浏览器凭本机保存的座位凭证入座，别人打开同一局只能旁观。

![真人座位](docs/images/human-seat.png)
...
- 🙋 **你也能上桌** —— 留一个座位给自己，和 11 个 agent 打一局；有真人时对局中不能开上帝视角。
- 🗣️ **发言有字数上限** —— 默认 450 字 ≈ 真人讲 2 分钟，超长会被打回重说。
  思考不设限，但每个决策点最多问 3 次，不会卡死。
- ▶️ **点一次就跑完** —— 开局后全自动，不需要再点任何东西（留了真人座位时，轮到你才会停下来等你）。
...
4. **点「天黑请闭眼」**：之后全自动，坐着看就行；想看狼人在密谋什么就打开「上帝视角」。
   如果给自己留了座位（后端选「真人玩家」），轮到你时页面上方会出现操作面板，这局结束前上帝视角是锁住的
